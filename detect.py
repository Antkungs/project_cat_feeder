...
def main():
    count = 0
    model = YOLO("model/yolov8x.pt") #model หาแมว
    modelCheck = torch.hub.load('ultralytics/yolov5','custom',"model/cat3.pt") #model แยกแยะแมวที่เทรนไว้
    ...
    while cap.isOpened():
        ...
        result = model(frame)
        try:      
            if(result[0].boxes.cls.tolist().count(15) > 1): #เช็คแมวว่ามี > 1 ตัว
                print("เจอแมวมากกว่า 1 ตัว")
                count = 0
                
            elif(result[0].boxes.cls.tolist().count(15) == 1): #เช็คแมวว่ามี 1 ตัว
